...
public class LoginController extends HttpServlet {
	...
	/**
	 * Se recogen nombre usuario, password, idioma a seleccionar se comprueba si
	 * esta en la BBDD. Se comprueba el Rol del usuario si Administrador se envia a
	 * "views/backoffice/inicio" si Usuario se envia a "views/frontoffice/inicio" Si
	 * no se encuentra o presenta un dato erroneo se envia a "views/login.jsp"
	 * 
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (usuario != null) {
			...
			if (usuario.getRol().getId() == Rol.ADMINISTRADOR) {
				// request.getRequestDispatcher("views/backoffice/inicio").forward(request,
				// response);
				response.sendRedirect("views/backoffice/inicio");
			} else {
				// request.getRequestDispatcher("views/frontoffice/inicio").forward(request,
				// response);
